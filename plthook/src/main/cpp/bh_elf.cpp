...
#define BH_ELF_IS_EXPORT_SYM(shndx) (SHN_UNDEF != (shndx))
...
bh_elf_iterate_aps2(bh_sleb128_decoder_t *decoder, bool (*callback)(Elf_Reloc *, void *),
    ...
    Elf_Reloc reloc;
    if (0 != bh_sleb128_decoder_next(decoder, (size_t *) &reloc.r_offset)) {
        return;
    }

    for (size_t idx = 0; idx < num_relocs;) {
        size_t group_size;
        if (0 != bh_sleb128_decoder_next(decoder, &group_size)) {
            return;
        }
        size_t group_flags;
        if (0 != bh_sleb128_decoder_next(decoder, &group_flags)) {
            return;
        }
        ...
        if (group_flags & RELOCATION_GROUPED_BY_OFFSET_DELTA_FLAG) {
            if (0 != bh_sleb128_decoder_next(decoder, &group_r_offset_delta)) {
                return;
            }
        }

        if (group_flags & RELOCATION_GROUPED_BY_INFO_FLAG) {
            if (0 != bh_sleb128_decoder_next(decoder, (size_t *) &reloc.r_info)) {
                return;
            }
        }
        ...
        for (size_t i = 0; i < group_size; ++i) {
            if (group_flags & RELOCATION_GROUPED_BY_OFFSET_DELTA_FLAG) {
                ...
                size_t val;
                if (0 != bh_sleb128_decoder_next(decoder, &val)) {
                    return;
                }
                reloc.r_offset += val;
            }
...
static void bh_elf_parse_dynamic_unsafe(bh_elf_t *self, ElfW(Dyn) *dynamic) {
    ...
    if (NULL != self->rel_dyn_aps2) {
        ...
        if (self->rel_dyn_aps2_sz < 4 || rel[0] != 'A' || rel[1] != 'P' || rel[2] != 'S' ||
            rel[3] != '2') {
            self->rel_dyn_aps2 = 0;
...
bh_elf_t *bh_elf_create(struct dl_phdr_info *info) {
    if (0 == info->dlpi_phdr || NULL == info->dlpi_name || NULL == info->dlpi_phdr ||
        0 == info->dlpi_phnum) {
        return NULL;
    }

    bh_elf_t *self;
    if (NULL == (self = static_cast<bh_elf_t *>(calloc(1, sizeof(bh_elf_t))))) {
        return NULL;
    }
...
static int bh_elf_get_protect_by_address_unsafe(bh_elf_t *self, void *address) {
    for (size_t i = 0; i < self->dlpi_phnum; i++) {
        const ElfW(Phdr) *phdr = &(self->dlpi_phdr[i]);
        if (self->dlpi_phdr[i].p_type == PT_GNU_RELRO) {
            if ((uintptr_t) address >= (self->load_bias + phdr->p_vaddr) &&
                (uintptr_t) address < (self->load_bias + phdr->p_vaddr + phdr->p_memsz)) {
                return PROT_READ;
            }
        }
    }
...
int bh_elf_get_protect_by_address(bh_elf_t *self, void *address) {
    ...
    BYTESIG_TRY(SIGSEGV, SIGBUS)
            {
                prot = bh_elf_get_protect_by_address_unsafe(self, address);
            }
        BYTESIG_CATCH()
            {
                self->error = true;
...
bh_elf_find_import_func_symbol_by_symbol_name(bh_elf_t *self, const char *sym_name) {
    ...
    if (self->sysv_hash.buckets_cnt > 0) {
        ...
        if (NULL != sym && BH_ELF_IS_IMPORT_SYM(sym->st_shndx)) {
            return sym;
        }
    }
    ...
    if (self->gnu_hash.buckets_cnt > 0) {
        ...
        if (NULL != sym && BH_ELF_IS_EXPORT_SYM(sym->st_shndx)) {
            return sym;
        }
    }
    ...
    for (size_t i = 0; i < self->rel_plt_cnt; i++) {
        ...
        if (0 == strcmp(self->dynstr + self->dynsym[sym_idx].st_name, sym_name)) {
            return &self->dynsym[sym_idx];
        }
    }
...
ElfW(Sym) *bh_elf_find_export_func_symbol_by_symbol_name(bh_elf_t *self, const char *sym_name) {
    ...
    if (0 != bh_elf_parse_dynamic(self)) {
        return NULL;
    }
...
static bool bh_elf_find_import_func_addr_by_symbol_name_unsafe_aps2_cb(Elf_Reloc *rel, void *arg) {
    ...
    if (BH_ELF_R_GLOB_DAT != BH_ELF_R_TYPE(rel->r_info) &&
        BH_ELF_R_ABS != BH_ELF_R_TYPE(rel->r_info)) {
        return true;
    }
...
bh_elf_find_import_func_addr_by_symbol_name_unsafe(bh_elf_t *self, const char *sym_name,
    ...
    for (size_t i = 0; i < self->rel_plt_cnt; i++) {
        ...
        if (BH_ELF_R_JUMP_SLOT != BH_ELF_R_TYPE(rel->r_info)) {
            continue;
        }
...
size_t bh_elf_find_import_func_address_by_symbol_name(bh_elf_t *self, const char *sym_name,
    ...
    BYTESIG_TRY(SIGSEGV, SIGBUS)
            {
                address_array_sz =
                        bh_elf_find_import_func_addr_by_symbol_name_unsafe(self, sym_name,
                                                                           address_array,
                                                                           address_array_cap);
            }
        BYTESIG_CATCH()
...
bh_elf_find_import_func_address_by_callee_address_unsafe_aps2_cb(Elf_Reloc *rel, void *arg) {
    ...
    if (*((void **) (self->load_bias + rel->r_offset)) != target_address) {
        return true;
    }
...
bh_elf_find_import_func_address_by_callee_address_unsafe(bh_elf_t *self, void *target_address,
    ...
    for (size_t i = 0; i < self->rel_dyn_cnt; i++) {
        ...
        address_array[address_array_sz++] = (void *) (self->load_bias + rel->r_offset);
        if (address_array_sz >= address_array_cap) return address_array_sz;
    }

    if (NULL != self->rel_dyn_aps2) {
        bh_sleb128_decoder_t decoder;
        bh_sleb128_decoder_init(&decoder, self->rel_dyn_aps2, self->rel_dyn_aps2_sz);
        void *pkg[5] = {self, target_address, (void *) address_array, (void *) address_array_cap,
                        &address_array_sz};
        bh_elf_iterate_aps2(&decoder,
                            bh_elf_find_import_func_address_by_callee_address_unsafe_aps2_cb, pkg);
    }

    return address_array_sz;
}

size_t bh_elf_find_import_func_address_by_callee_address(bh_elf_t *self, void *target_address,
                                                         void **address_array,
                                                         size_t address_array_cap) {
    if (self->error) {
        return 0;
    ...
    BYTESIG_TRY(SIGSEGV, SIGBUS)
            {
                address_array_sz =
                        bh_elf_find_import_func_address_by_callee_address_unsafe(self,
                                                                                 target_address,
                                                                                 address_array,
                                                                                 address_array_cap);
            }
...
void *bh_elf_find_export_func_address_by_symbol_name(bh_elf_t *self, const char *sym_name) {
    ...
    BYTESIG_TRY(SIGSEGV, SIGBUS)
            {
                address = bh_elf_find_export_func_address_by_symbol_name_unsafe(self, sym_name);
            }
        BYTESIG_CATCH()
            {
                self->error = true;
