...
static int bh_dl_iterate_by_maps(int (*callback)(struct dl_phdr_info *, size_t, void *), void *data) {
    ...
    while (fgets(line, sizeof(buf1), maps)) {
        if (!bh_util_ends_with(line, BH_CONST_BASENAME_APP_PROCESS) && !bh_util_ends_with(line, ".so")) {
            continue;
        }
        ...
        if ('-' == exec && 0 == offset) {
            ...
            if (try_next_line && 0 != offset) {
                ...
                if (0 != strcmp(prev_pathname, pathname)) {
                    goto clean;
                }
            ...
            if (UINTPTR_MAX == min_vaddr) {
                goto clean;  // ignore invalid ELF
            }
            info.dlpi_addr = (ElfW(Addr))(base - min_vaddr);

            // callback
            if (0 != callback(&info, sizeof(struct dl_phdr_info), data)) {
                break;
            }
        }
