...
static void *bh_dl_read_to_memory(int file_fd, size_t file_sz, size_t data_offset, size_t data_len) {
    ...
    if (data_offset + data_len > file_sz) {
        return NULL;
    }
...
void *bh_dl_open_linker(void ) {
...
#if __ANDROID_API__ < __ANDROID_API_J_MR2__
    if (0 == base) {
        base = bh_dl_find_linker_base_from_maps();
    }
#endif
    ...
    if (UINTPTR_MAX == min_vaddr || base < min_vaddr) {
        return NULL;
    }
    ...
    bh_dl_t *self;
    if (NULL == (self = static_cast<bh_dl_t *>(calloc(1, sizeof(bh_dl_t))))) {
        return NULL;
    }
...
void *bh_dl_dsym(void *handle, const char *symbol) {
    ...
    for (size_t i = 0; i < self->symtab_cnt; ++i) {
        ...
        if (!BH_DL_SYMTAB_IS_EXPORT_SYM(sym->st_shndx)) {
            continue;
        }
        if (0 != strncmp(self->strtab + sym->st_name, symbol, self->strtab_sz - sym->st_name)) {
            continue;
        }
