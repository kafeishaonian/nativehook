...
int sh_util_mprotect(uintptr_t address, size_t len, int prot) {
    uintptr_t start = SH_UTIL_PAGE_START(address);
    uintptr_t end = SH_UTIL_PAGE_END(address + len - 1);
...
bool sh_util_is_thumb32(uintptr_t target_address) {
    uint16_t opcode = *((uint16_t *) target_address);
    int tmp = opcode >> 11u;
...
int sh_util_write_inst(uintptr_t target_address, void *inst, size_t inst_len) {
    if (0 != sh_util_mprotect(target_address, inst_len, PROT_READ | PROT_WRITE | PROT_EXEC)) {
        return SHADOWHOOK_ERRNO_MPROT;
    }

    SH_SIG_TRY(SIGSEGV, SIGBUS)
            {
                if ((4 == inst_len) & (0 == target_address % 4)) {
                    __atomic_store_n((uint32_t *) target_address, *((uint32_t *) inst),
                                     __ATOMIC_SEQ_CST);
                } else if ((8 == inst_len) && (0 == target_address % 8)) {
                    __atomic_store_n((uint64_t *) target_address, *((uint64_t *) inst),
                                     __ATOMIC_SEQ_CST);
                }
#ifdef __LP64__
                    else if ((16 == inst_len) && (0 == target_address % 16)) {
                        __atomic_store_n((__int128 *)target_addr, *((__int128 *)inst), __ATOMIC_SEQ_CST);
                    }
#endif
                else {
                    memcpy((void *) target_address, inst, inst_len);
                }

                sh_util_clear_cache(target_address, inst_len);
            }
...
int sh_util_get_api_level(void) {
    ...
    if (__predict_false(xdl_util_api_level < 0)) {
        ...
        if (__predict_false(api_level < 0)) {
            api_level = sh_util_get_api_level_from_build_prop();
        }
        if (__predict_false(api_level < __ANDROID_API_J__)) {
...
struct tm *sh_util_localtime_r(const time_t *timep, long gmtoff, struct tm *result) {
    ...
    while (days < 0 || days >= (SH_UTIL_ISLEAP(y) ? 366 : 365)) {
        ...
        days -= ((yg - y) * 365 + SH_UTIL_LEAPS_THRU_END_OF(yg - 1) -
...
static void sh_util_format_unsigned(
        char *buf, size_t buf_size, uint64_t value, int base, int caps) {
