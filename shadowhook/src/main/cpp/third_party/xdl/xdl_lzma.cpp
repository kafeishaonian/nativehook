...
typedef int (*xdl_lzma_code_t)(void *, uint8_t *, size_t *, const uint8_t *, size_t *,
                               ECoderFinishMode,
                               ECoderStatus *);
...
static void xdl_lzma_init(void) {
    ...
    if (NULL == (crcgen = (xdl_lzma_crcgen_t) xdl_sym(lzma, XDL_LZMA_SYM_CRCGEN, NULL))) {
        goto end;
    }
    if (NULL == (crc64gen = (xdl_lzma_crc64gen_t) xdl_sym(lzma, XDL_LZMA_SYM_CRC64GEN, NULL))) {
        goto end;
    }
    if (NULL ==
        (xdl_lzma_construct = (xdl_lzma_construct_t) xdl_sym(lzma, XDL_LZMA_SYM_CONSTRUCT, NULL))) {
        goto end;
    }
    if (NULL ==
        (xdl_lzma_isfinished = (xdl_lzma_isfinished_t) xdl_sym(lzma, XDL_LZMA_SYM_ISFINISHED,
                                                               NULL))) {
        goto end;
    }
    if (NULL == (xdl_lzma_free = (xdl_lzma_free_t) xdl_sym(lzma, XDL_LZMA_SYM_FREE, NULL))) {
        goto end;
    }
    if (NULL == (xdl_lzma_code = xdl_sym(lzma, XDL_LZMA_SYM_CODE, NULL))) {
        goto end;
    }
    crcgen();
    crc64gen();
...
int xdl_lzma_decompress(uint8_t *src, size_t src_size, uint8_t **dst, size_t *dst_size) {
    ...
    *dst = NULL;
    do {
        *dst_size *= 2;
        if (NULL == (*dst = static_cast<uint8_t *>(realloc(*dst, *dst_size)))) {
            xdl_lzma_free(&state);
            return -1;
        }
        ...
        if (api_level >= __ANDROID_API_Q__) {
            ...
            result = lzma_code_q(&state, *dst + dst_offset, &dst_remaining, src + src_offset,
                                 &src_remaining, 1,
    ...
    *dst_size = dst_offset;
    *dst = static_cast<uint8_t *>(realloc(*dst, *dst_size));
    return 0;
}