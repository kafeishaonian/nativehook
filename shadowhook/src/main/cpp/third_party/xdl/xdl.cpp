...
xdl_read_file_to_heap(int file_fd, size_t file_sz, size_t data_offset, size_t data_len) {
    ...
    if (data_offset != (size_t) lseek(file_fd, (off_t) data_offset, SEEK_SET)) {
        return NULL;
    }
...
static void *xdl_get_memory(void *mem, size_t mem_sz, size_t data_offset, size_t data_len) {
    ...
    if (data_offset >= mem_sz) {
        return NULL;
    }
    if (data_offset + data_len > mem_sz) {
        return NULL;
    }
...
static int xdl_symtab_load_from_debugdata(xdl_t *self, int file_fd, size_t file_sz,
    ...
    // get ELF header
    ehdr = (ElfW(Ehdr) *) debugdata;
    if (0 == ehdr->e_shnum || ehdr->e_shentsize != sizeof(ElfW(Shdr))) {
        goto end;
    }
    ...
    shstrtab = (char *) xdl_get_memory_by_section(debugdata, debugdata_sz,
...
static xdl_t *xdl_find_from_auxv(unsigned long type, const char *pathname) {
    ...
    // create xDL object
    xdl_t *self;
    if (NULL == (self = static_cast<xdl_t *>(calloc(1, sizeof(xdl_t))))) return NULL;
...
static int xdl_find_iterate_cb(struct dl_phdr_info *info, size_t size, void *arg) {
    ...
    if (NULL == ((*self) = static_cast<xdl_t *>(calloc(1, sizeof(xdl_t)))))
...
static int xdl_open_by_addr_iterate_cb(struct dl_phdr_info *info, size_t size, void *arg) {
    ...
    if (xdl_elf_is_match(info->dlpi_addr, info->dlpi_phdr, info->dlpi_phnum, addr)) {
        // found the target ELF
        if (NULL == ((*self) = static_cast<xdl_t *>(calloc(1, sizeof(xdl_t))))) return 1;  // failed
        if (NULL == ((*self)->pathname = strdup((const char *) info->dlpi_name))) {
            free(*self);
            *self = NULL;
            return 1;  // failed
...
int xdl_info(void *handle, int request, void *info) {
    if (NULL == handle || XDL_DI_DLINFO != request || NULL == info) {
        return -1;
    }
