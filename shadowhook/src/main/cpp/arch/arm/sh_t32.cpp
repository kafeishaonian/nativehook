...
static size_t
sh_t32_rewrite_b(uint16_t *buffer, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
    ...
    uint32_t address;
    if (type == B_T3) {
        uint32_t x = (s << 20u) | (j2 << 19u) | (j1 << 18u) | ((high_inst & 0x3Fu) << 12u) |
                     ((low_inst & 0x7FFu) << 1u);
        uint32_t imm32 = SH_UTIL_SIGN_EXTEND_32(x, 21u);
        address = SH_UTIL_SET_BIT0(pc + imm32);
    } else if (type == B_T4) {
        uint32_t x =
                (s << 24u) | (i1 << 23u) | (i2 << 22u) | ((high_inst & 0x3FFu) << 12u) |
                ((low_inst & 0x7FFu) << 1u);
        uint32_t imm32 = SH_UTIL_SIGN_EXTEND_32(x, 25u);
        address = SH_UTIL_SET_BIT0(pc + imm32);  // thumb -> thumb
        ...
        address = SH_UTIL_ALIGN_4(pc) + imm32;  // thumb -> arm, align4
    }
    address = sh_txx_fix_address(address, rinfo);

    size_t idx = 0;
    if (type == B_T3) {
        uint32_t cond = SH_UTIL_GET_BITS_16(high_inst, 9, 6);
        buffer[idx++] = 0xD000u | (uint16_t) (cond << 8u);  // B<c> #0
        buffer[idx++] = 0xE003;                            // B #6
    } else if (type == BL_IMM_T1 || type == BLX_IMM_T2) {
        buffer[idx++] = 0xF20F;  // ADD LR, PC, #9
        buffer[idx++] = 0x0E09;  // ...
    }
    buffer[idx++] = 0xF8DF;  // LDR.W PC, [PC]
    buffer[idx++] = 0xF000;  // ...
    buffer[idx++] = address & 0xFFFFu;
    buffer[idx++] = address >> 16u;
...
static size_t sh_t32_rewrite_adr(uint16_t *buf, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
    ...
    uint32_t addr = (type == ADR_T2 ? (SH_UTIL_ALIGN_4(pc) - imm32) : (SH_UTIL_ALIGN_4(pc) +
                                                                       imm32));
    if (sh_txx_is_address_need_fix(addr, rinfo)) {
        return 0;  // rewrite failed
    }
...
static size_t sh_t32_rewrite_pl(uint16_t *buf, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
    ...
    addr = sh_txx_fix_address(addr, rinfo);
...
static size_t sh_t32_rewrite_tb(uint16_t *buf, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
    ...
    if (target_addr + 4 != rinfo->end_address) {
        return 0;  // rewrite failed
    }
...
static size_t
sh_t32_rewrite_vldr(uint16_t *buf, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
...
size_t sh_t32_rewrite(uint16_t *buf, uint16_t high_inst, uint16_t low_inst, uintptr_t pc,
    ...
    SH_LOG_INFO("t32 rewrite: type %d, high inst %" PRIx16 ", low inst %" PRIx16, type, high_inst,
                low_inst);
    ...
    else if (type == LDR_LIT_T2 || type == LDR_LIT_PC_T2 || type == LDRB_LIT_T1 ||
             type == LDRD_LIT_T1 ||
