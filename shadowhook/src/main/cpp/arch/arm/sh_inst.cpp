...
static void sh_inst_get_thumb_rewrite_info(sh_inst_t *self, uintptr_t target_address,
                                           sh_txx_rewrite_info_t *rinfo) {
    memset(rinfo, 0, sizeof(sh_txx_rewrite_info_t));

    size_t idx = 0;
    uintptr_t target_address_offset = 0;
    uintptr_t pc = target_address + 4;
    size_t rewrite_len = 0;

    while (rewrite_len < self->backup_len) {
        sh_t16_it_t it;
        if (sh_t16_parse_it(&it, *((uint16_t *) (target_address + target_address_offset)), pc)) {
            rewrite_len += (2 + it.insts_len);

            size_t it_block_idx = idx++;
            size_t it_block_len = 4 + 4;
            for (size_t i = 0, j = 0; i < it.insts_cnt; ++i) {
                bool is_thumb32 = sh_util_is_thumb32((uintptr_t) (&(it.insts[j])));
                if (is_thumb32) {
                    it_block_len += sh_t32_get_rewrite_inst_len(it.insts[j], it.insts_cnt[j + 1]);
            ...
            target_address_offset += (2 + it.insts_len);
            pc += (2 + it.insts_len);
        } else {
            bool is_thumb32 = sh_util_is_thumb32(target_address + target_address_offset);
            size_t inst_len = (is_thumb32 ? 4 : 2);
            rewrite_len += inst_len;

            if (is_thumb32) {
                rinfo->inst_lens[idx++] = sh_t32_get_rewrite_inst_len(
                        *((uint16_t *) (target_address + target_address_offset)),
                        *((uint16_t *) (target_address + target_address_offset + 2)));
                rinfo->inst_lens[idx++] = 0;
            } else {
                rinfo->inst_lens[idx++] = sh_t16_get_rewrite_inst_len(
                        *((uint16_t *) (target_address + target_address_offset)));

            }

            target_address_offset += inst_len;
            pc += inst_len;
        }
    }

    rinfo->start_address = target_address;
    rinfo->end_address = target_address + rewrite_len;
    rinfo->buffer = (uint16_t *) self->enter_address;
    rinfo->buffer_offset = 0;
    rinfo->inst_lens_cnt = idx;
}

static int
sh_inst_hook_thumb_rewrite(sh_inst_t *self, uintptr_t target_address, uintptr_t *orig_address,
                           uintptr_t *orig_address2, size_t *rewrite_len) {

    memcpy((void *) (self->backup), (void *) target_address, self->backup_len);

    sh_txx_rewrite_info_t rinfo;
    sh_inst_get_thumb_rewrite_info(self, target_address, &rinfo);
    ...
    *rewrite_len = 0;
    ...
    while (*rewrite_len < self->backup_len) {
        ...
        if (sh_t16_parse_it(&it, *((uint16_t *) (target_address + target_address_offset)), pc)) {
            *rewrite_len += (2 + it.insts_len);
            ...
            uintptr_t enter_inst_else_p = self->enter_address + rinfo.buffer_offset;
            rinfo.buffer_offset += 2;  // B<c> <label>
            rinfo.buffer_offset += 2;  // NOP
            ...
            for (size_t i = 0, j = 0; i < it.insts_cnt; i++) {
                if (i == it.insts_else_cnt) {
                    ...
                    enter_inst_then_p = self->enter_address + rinfo.buffer_offset;
                    rinfo.buffer_offset += 2;  // B <label>
                    rinfo.buffer_offset += 2;  // NOP

                    // fill IT-else B instruction
                    sh_t16_rewrite_it_else((uint16_t *) enter_inst_else_p,
                                           (uint16_t) enter_inst_else_len, &it);
                }
                ...
                size_t len;
                if (is_thumb32)
                    len = sh_t32_rewrite((uint16_t *) (self->enter_address + rinfo.buffer_offset),
                                         it.insts[j],
                                         it.insts[j + 1], it.pcs[i], &rinfo);
                else
                    len = sh_t16_rewrite((uint16_t *) (self->enter_address + rinfo.buffer_offset),
                                         it.insts[j], it.pcs[i],
                                         &rinfo);
                if (0 == len) return SHADOWHOOK_ERRNO_HOOK_REWRITE_FAILED;
                rinfo.buffer_offset += len;
                j += (is_thumb32 ? 2 : 1);
                ...
                if (i == it.insts_cnt - 1) {
                    ...
                    sh_t16_rewrite_it_then((uint16_t *) enter_inst_then_p,
                                           (uint16_t) enter_inst_then_len);
                }
            }
            ...
            *rewrite_len += inst_len;
            ...
            SH_LOG_INFO("thumb rewrite: offset %zu, pc %" PRIxPTR, rinfo.buffer_offset, pc);
            size_t len;
            if (is_thumb32) {
                len = sh_t32_rewrite((uint16_t *) (self->enter_address + rinfo.buffer_offset),
                                     *((uint16_t *) (target_address + target_address_offset)),
                ...
                len = sh_t16_rewrite((uint16_t *) (self->enter_address + rinfo.buffer_offset),
                                     *((uint16_t *) (target_address + target_address_offset)), pc,
                                     &rinfo);
            }
            if (0 == len) return SHADOWHOOK_ERRNO_HOOK_REWRITE_FAILED;
            rinfo.buffer_offset += len;
    ...
    SH_LOG_INFO("thumb rewrite: len %zu to %zu", *rewrite_len, rinfo.buffer_offset);

    // absolute jump back to remaining original instructions (fill in enter)
    rinfo.buffer_offset += sh_t32_absolute_jump(
            (uint16_t *) (self->enter_address + rinfo.buffer_offset), true,
            SH_UTIL_SET_BIT0(target_address + *rewrite_len));
    sh_util_clear_cache(self->enter_address, rinfo.buffer_offset);

    // save original function address
    if (NULL != orig_address)
        __atomic_store_n(orig_address, SH_UTIL_SET_BIT0(self->enter_address), __ATOMIC_SEQ_CST);
    if (NULL != orig_address2)
        __atomic_store_n(orig_address2, SH_UTIL_SET_BIT0(self->enter_address), __ATOMIC_SEQ_CST);
    return 0;
}
...
static bool
sh_inst_thumb_is_long_enough(uintptr_t target_addr, size_t overwrite_len, xdl_info_t *dlinfo) {
...
static int sh_inst_hook_thumb_with_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    if (0 != (r = sh_exit_alloc(&self->exit_address, &self->exit_type, pc, dlinfo,
                                (uint8_t *) (self->exit),
                                sizeof(self->exit), SH_INST_T32_B_RANGE_LOW,
                                SH_INST_T32_B_RANGE_HIGH)))
        return r;
    ...
    SH_SIG_TRY(SIGSEGV, SIGBUS)
            {
                r = sh_inst_hook_thumb_rewrite(self, target_addr, orig_addr, orig_addr2,
                                               &rewrite_len);
            }
        SH_SIG_CATCH()
            {
    ...
    sh_t32_relative_jump((uint16_t *) self->trampo, self->exit_address, pc);
    __atomic_thread_fence(__ATOMIC_SEQ_CST);
    if (0 != (r = sh_util_write_inst(target_addr, self->trampo, self->backup_len))) goto err;

    SH_LOG_INFO(
            "thumb: hook (WITH EXIT) OK. target %" PRIxPTR " -> exit %" PRIxPTR " -> new %" PRIxPTR
            " -> enter %" PRIxPTR " -> remaining %" PRIxPTR,
            target_addr, self->exit_address, new_addr, self->enter_address,
            SH_UTIL_SET_BIT0(target_addr + rewrite_len));
    return 0;

    err:
    sh_exit_free(self->exit_address, self->exit_type, (uint8_t *) (self->exit), sizeof(self->exit));
    self->exit_address = 0;  // this is a flag for with-exit or without-exit
    return r;
}
...
static int
sh_inst_hook_thumb_without_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    SH_LOG_INFO(
            "thumb: hook (WITHOUT EXIT) OK. target %" PRIxPTR " -> new %" PRIxPTR " -> enter %" PRIxPTR
            " -> remaining %" PRIxPTR,
            target_addr, new_addr, self->enter_address,
            SH_UTIL_SET_BIT0(target_addr + rewrite_len));
    return 0;
}
...
static int sh_inst_hook_arm_rewrite(sh_inst_t *self, uintptr_t target_addr, uintptr_t *orig_addr,
    ...
    rinfo.rewrite_buf = (uint32_t *) self->enter_address;
    rinfo.rewrite_buf_offset = 0;
    rinfo.rewrite_inst_lens_cnt = self->backup_len / 4;
    for (uintptr_t i = 0; i < self->backup_len; i += 4)
        rinfo.rewrite_inst_lens[i / 4] = sh_a32_get_rewrite_inst_len(
                *((uint32_t *) (target_addr + i)));

    // rewrite original instructions (fill in enter)
    uintptr_t pc = target_addr + 8;
    for (uintptr_t i = 0; i < self->backup_len; i += 4, pc += 4) {
        size_t offset = sh_a32_rewrite(
                (uint32_t *) (self->enter_address + rinfo.rewrite_buf_offset),
    ...
    // absolute jump back to remaining original instructions (fill in enter)
    rinfo.rewrite_buf_offset += sh_a32_absolute_jump(
            (uint32_t *) (self->enter_address + rinfo.rewrite_buf_offset),
            target_addr + self->backup_len);
    sh_util_clear_cache(self->enter_address, rinfo.rewrite_buf_offset);

    // save original function address
    if (NULL != orig_addr) __atomic_store_n(orig_addr, self->enter_address, __ATOMIC_SEQ_CST);
    if (NULL != orig_addr2) __atomic_store_n(orig_addr2, self->enter_address, __ATOMIC_SEQ_CST);
    return 0;
}
...
static int sh_inst_hook_arm_with_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    if (0 != (r = sh_exit_alloc(&self->exit_address, &self->exit_type, pc, dlinfo,
                                (uint8_t *) (self->exit),
                                sizeof(self->exit), SH_INST_A32_B_RANGE_LOW,
                                SH_INST_A32_B_RANGE_HIGH)))
        return r;
    ...
    // relative jump to the exit by overwriting the head of original function
    sh_a32_relative_jump(self->trampo, self->exit_address, pc);
    __atomic_thread_fence(__ATOMIC_SEQ_CST);
    if (0 != (r = sh_util_write_inst(target_addr, self->trampo, self->backup_len))) goto err;

    SH_LOG_INFO(
            "a32: hook (WITH EXIT) OK. target %" PRIxPTR " -> exit %" PRIxPTR " -> new %" PRIxPTR
            " -> enter %" PRIxPTR " -> remaining %" PRIxPTR,
            target_addr, self->exit_address, new_addr, self->enter_address,
            target_addr + self->backup_len);
    return 0;
...
static int sh_inst_hook_arm_without_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    SH_LOG_INFO(
            "a32: hook (WITHOUT EXIT) OK. target %" PRIxPTR " -> new %" PRIxPTR " -> enter %" PRIxPTR
            " -> remaining %" PRIxPTR,
            target_addr, new_addr, self->enter_address, target_addr + self->backup_len);
    return 0;
}
...
int sh_inst_hook(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo, uintptr_t new_addr,
    ...
    self->enter_address = sh_enter_alloc();
    if (0 == self->enter_address) return SHADOWHOOK_ERRNO_HOOK_ENTER;

    int r;
    if (SH_UTIL_IS_THUMB(target_addr)) {
#ifdef SH_CONFIG_TRY_WITH_EXIT
        if (0 == (r = sh_inst_hook_thumb_with_exit(self, target_addr, dlinfo, new_addr, orig_addr,
                                                   orig_addr2)))
            return r;
#endif
        if (0 ==
            (r = sh_inst_hook_thumb_without_exit(self, target_addr, dlinfo, new_addr, orig_addr,
                                                 orig_addr2)))
            return r;
    } else {
#ifdef SH_CONFIG_TRY_WITH_EXIT
        if (0 == (r = sh_inst_hook_arm_with_exit(self, target_addr, dlinfo, new_addr, orig_addr,
                                                 orig_addr2)))
            return r;
#endif
        if (0 == (r = sh_inst_hook_arm_without_exit(self, target_addr, dlinfo, new_addr, orig_addr,
                                                    orig_addr2)))
            return r;
    }
    ...
    sh_enter_free(self->enter_address);
    return r;
}
...
int sh_inst_unhook(sh_inst_t *self, uintptr_t target_addr) {
    ...
    if (0 != self->exit_address)
        if (0 !=
            (r = sh_exit_free(self->exit_address, self->exit_type, (uint8_t *) (self->exit),
                              sizeof(self->exit))))
            return r;
