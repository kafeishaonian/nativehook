...
static size_t sh_a64_rewrite_adr(uint32_t *buf, uint32_t inst, uintptr_t pc, sh_a64_type_t type,
    ...
    if (type == ADR)
        ...
        addr = (pc & 0xFFFFFFFFFFFFF000) +
               SH_UTIL_SIGN_EXTEND_64((immhi << 14u) | (immlo << 12u), 33u);
...
static size_t
sh_a64_rewrite_cb(uint32_t *buf, uint32_t inst, uintptr_t pc, sh_a64_rewrite_info_t *rinfo) {
    uint64_t imm19 = SH_UTIL_GET_BITS_32(inst, 23, 5);
    uint64_t offset = SH_UTIL_SIGN_EXTEND_64((imm19 << 2u), 21u);
    uint64_t addr = pc + offset;
    addr = sh_a64_fix_addr(addr, rinfo);
...
static size_t
sh_a64_rewrite_tb(uint32_t *buf, uint32_t inst, uintptr_t pc, sh_a64_rewrite_info_t *rinfo) {
