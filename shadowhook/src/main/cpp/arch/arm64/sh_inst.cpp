...
static int sh_inst_hook_rewrite(sh_inst_t *self, uintptr_t target_addr, uintptr_t *orig_addr,
    ...
    rinfo.buf = (uint32_t *) self->enter_address;
    rinfo.buf_offset = 0;
    ...
    for (uintptr_t i = 0; i < self->backup_len; i += 4, pc += 4) {
        size_t offset = sh_a64_rewrite((uint32_t *) (self->enter_address + rinfo.buf_offset),
    ...
    rinfo.buf_offset += sh_a64_absolute_jump_with_ret(
            (uint32_t *) (self->enter_address + rinfo.buf_offset),
            target_addr + self->backup_len);
    sh_util_clear_cache(self->enter_address, rinfo.buf_offset);
...
static int sh_inst_hook_with_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    if (0 !=
        (r = sh_exit_alloc(&self->exit_address, (uint16_t *) &self->exit_type, pc, dlinfo,
                           (uint8_t *) (self->exit),
    ...
    sh_a64_relative_jump(self->trampo, self->exit_address, pc);
    __atomic_thread_fence(__ATOMIC_SEQ_CST);
    if (0 != (r = sh_util_write_inst(target_addr, self->trampo, self->backup_len))) goto err;

    SH_LOG_INFO(
            "a64: hook (WITH EXIT) OK. target %" PRIxPTR " -> exit %" PRIxPTR " -> new %" PRIxPTR
    ...
    err:
    sh_exit_free(self->exit_address, (uint16_t) self->exit_type, (uint8_t *) (self->exit),
                 sizeof(self->exit));
...
static int sh_inst_hook_without_exit(sh_inst_t *self, uintptr_t target_addr, xdl_info_t *dlinfo,
    ...
    SH_LOG_INFO(
            "a64: hook (WITHOUT EXIT) OK. target %" PRIxPTR " -> new %" PRIxPTR " -> enter %" PRIxPTR
            " -> remaining %" PRIxPTR,
...
#ifdef SH_CONFIG_TRY_WITH_EXIT
    if (0 ==
        (r = sh_inst_hook_with_exit(self, target_addr, dlinfo, new_addr, orig_addr, orig_addr2)))
        return r;
#endif
    if (0 ==
        (r = sh_inst_hook_without_exit(self, target_addr, dlinfo, new_addr, orig_addr, orig_addr2)))
        return r;
...
int sh_inst_unhook(sh_inst_t *self, uintptr_t target_addr) {
    ...
    if (0 != self->exit_address)
        if (0 != (r = sh_exit_free(self->exit_address, (uint16_t) self->exit_type,
                                   (uint8_t *) (self->exit),
                                   sizeof(self->exit))))
            return r;
