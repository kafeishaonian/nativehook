...
static bool sh_linker_check_arch(xdl_info_t *dlinfo) {
    ElfW(Ehdr) *ehdr = (ElfW(Ehdr) *) sh_linker_get_base_address(dlinfo);
...
int sh_linker_init(void) {
    ...
    if (__predict_true(api_level >= __ANDROID_API_L__)) {
        ...
        sh_linker_g_dl_mutex = (pthread_mutex_t *) (xdl_dsym(handle, SH_LINKER_SYM_G_DL_MUTEX,
                                                             NULL));
        if (NULL == sh_linker_g_dl_mutex && api_level >= __ANDROID_API_U__) {
            sh_linker_g_dl_mutex = (pthread_mutex_t *) (xdl_dsym(handle,
                                                                 SH_LINKER_SYM_G_DL_MUTEX_U_QPR2,
                                                                 NULL));
        }
        ...
        sh_linker_dlopen_dlinfo.dli_saddr = xdl_dsym(handle, sh_linker_dlopen_dlinfo.dli_sname,
                                                     &(sh_linker_dlopen_dlinfo.dli_ssize));
    ...
    return (0 != sh_linker_dlopen_addr &&
            (NULL != sh_linker_g_dl_mutex || api_level < __ANDROID_API_L__)) ? 0 : -1;
}
...
bool sh_linker_need_to_hook_dlopen(uintptr_t target_address) {
    return SHADOWHOOK_IS_UNIQUE_MODE && !sh_linker_dlopen_hooked &&
           target_address == sh_linker_dlopen_addr;
}
...
static void *sh_linker_proxy_dlopen(const char *filename, int flag) {
    ...
    if (SHADOWHOOK_IS_SHARED_MODE)
        handle = SHADOWHOOK_CALL_PREV(sh_linker_proxy_dlopen, sh_linker_proxy_dlopen_t, filename,
                                      flag);
    else
...
static void *sh_linker_proxy_do_dlopen_l(const char *name, int flags, const void *extinfo) {
    void *handle;
    if (SHADOWHOOK_IS_SHARED_MODE)
        handle = SHADOWHOOK_CALL_PREV(sh_linker_proxy_do_dlopen_l, sh_linker_proxy_do_dlopen_l_t,
                                      name, flags,
                                      extinfo);
    else
...
static void *sh_linker_proxy_do_dlopen_n(const char *name, int flags, const void *extinfo,
    ...
    void *handle;
    if (SHADOWHOOK_IS_SHARED_MODE)
        handle = SHADOWHOOK_CALL_PREV(sh_linker_proxy_do_dlopen_n, sh_linker_proxy_do_dlopen_n_t,
                                      name, flags,
                                      extinfo, caller_addr);
    else
...
int sh_linker_hook_dlopen(sh_linker_post_dlopen_t post_dlopen, void *post_dlopen_arg) {
    ...
    if (sh_linker_dlopen_hooked) {
        return result;
    }
    pthread_mutex_lock(&lock);

    int
    (*hook)(uintptr_t, uintptr_t, uintptr_t *, size_t *, xdl_info_t *) = SHADOWHOOK_IS_SHARED_MODE
                                                                         ? sh_switch_hook
                                                                         : sh_switch_hook_invisible;
    int api_level = sh_util_get_api_level();
    size_t backup_len = 0;
    int r;
    ...
    if (api_level < __ANDROID_API_L__) {
        // get & check dlinfo
        r = sh_linker_get_dlinfo_by_address((void *) sh_linker_dlopen_addr,
                                            &sh_linker_dlopen_dlinfo, NULL, 0, NULL,
                                            0, false);
        if (SHADOWHOOK_ERRNO_LINKER_ARCH_MISMATCH == r)
            result = SHADOWHOOK_ERRNO_LINKER_ARCH_MISMATCH;
        if (0 != r) goto end;

        // hook
        r = hook(sh_linker_dlopen_addr, (uintptr_t) sh_linker_proxy_dlopen,
                 (uintptr_t *) &sh_linker_orig_dlopen,
...
int sh_linker_get_dlinfo_by_address(void *address, xdl_info_t *dlinfo, char *lib_name,
                                    size_t lib_name_sz,
    ...
    if (sh_util_get_api_level() >= __ANDROID_API_L__) {
        r = xdl_addr((void *) address, dlinfo, &dlcache);
    } else {
    ...
    SH_LOG_INFO(
            "task: get dlinfo by target addr: target_addr %p, sym_name %s, sym_sz %zu, load_bias %" PRIxPTR
            ", pathname %s",
            address, NULL == dlinfo->dli_sname ? "(NULL)" : dlinfo->dli_sname, dlinfo->dli_ssize,
            (uintptr_t) dlinfo->dli_fbase,
            NULL == dlinfo->dli_fname ? "(NULL)" : dlinfo->dli_fname);
    ...
    if (NULL == dlinfo->dli_sname) {
        if (ignore_symbol_check) {
            dlinfo->dli_saddr = address;
            dlinfo->dli_sname = "unknown";
            dlinfo->dli_ssize = 1024;
        } else {
            const char *matched_dlfcn_name = NULL;
            if (NULL == (matched_dlfcn_name = sh_linker_match_dlfcn((uintptr_t) address))) {
                r = SHADOWHOOK_ERRNO_HOOK_DLINFO;
                goto end;
            } else {
                dlinfo->dli_saddr = address;
                ...
                dlinfo->dli_ssize = 4;
                SH_LOG_INFO("task: match dlfcn, target_addr %p, sym_name %s", address,
                            matched_dlfcn_name);
            }
        }
    }
    ...
    if (NULL != lib_name) {
        strlcpy(lib_name, dlinfo->dli_fname, lib_name_sz);
    }

    if (NULL != sym_name) {
        strlcpy(sym_name, dlinfo->dli_sname, sym_name_sz);
    }
...
int sh_linker_get_dlinfo_by_sym_name(const char *lib_name, const char *sym_name, xdl_info_t *dlinfo,
    ...
    if (crashed) {
        return SHADOWHOOK_ERRNO_HOOK_DLOPEN_CRASH;
    }
    if (NULL == handle) {
        return SHADOWHOOK_ERRNO_PENDING;
    }
    ...
    if (crashed) {
        return SHADOWHOOK_ERRNO_HOOK_DLSYM_CRASH;
    }
    if (NULL == addr) {
        return SHADOWHOOK_ERRNO_HOOK_DLSYM;
    }
    ...
    if (NULL != real_lib_name) {
        strlcpy(real_lib_name, dlinfo->dli_fname, real_lib_name_sz);
    }
    return 0;
}