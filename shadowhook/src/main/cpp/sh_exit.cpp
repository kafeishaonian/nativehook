...
static sh_exit_elfinfo_t sh_exit_vdso_info;
...
static void sh_exit_init_elfinfo(unsigned long type, sh_exit_elfinfo_t *info) {
    ...
    uintptr_t min_vaddr;
    uintptr_t load_bias;
    if (__predict_false(NULL == getauxval)) {
        goto err;
    }

    val = (uintptr_t) getauxval(type);
    if (__predict_false(0 == val)) {
        goto err;
    }

    base = (AT_PHDR == type ? (val & (~0xffful)) : val);
    if (__predict_false(0 != memcmp((void *) base, ELFMAG, SELFMAG))) {
        goto err;
    }

    ehdr = (ElfW(Ehdr) *) base;
    dlpi_phdr = (const ElfW(Phdr) *) (base + ehdr->e_phoff);
    dlpi_phnum = ehdr->e_phnum;

    min_vaddr = UINTPTR_MAX;
    ...
    if (__predict_false(UINTPTR_MAX == min_vaddr || base < min_vaddr)) {
        goto err;
    }
...
static int
sh_exit_alloc_out_library(uintptr_t *exit_address, uintptr_t pc, xdl_info_t *dlinfo, uint8_t *exit,
                          size_t exit_len, size_t range_low, size_t range_high) {
    uintptr_t address = sh_trampo_alloc(&sh_exit_trampo_mgr, pc, range_low, range_high);
    if (0 == address) {
        return -1;
    }

    memcpy((void *) address, exit, exit_len);
    sh_util_clear_cache(address, exit_len);
    *exit_address = address;
    return 0;
}

static void sh_exit_free_out_library(uintptr_t exit_address) {
    sh_trampo_free(&sh_exit_trampo_mgr, exit_address);
}
...
static size_t sh_exit_get_gaps(xdl_info_t *dlinfo, sh_exit_gap_t *gaps, size_t gaps_cap,
    ...
    for (size_t i = 0; i < dlinfo->dlpi_phnum; ++i) {
        ...
        uintptr_t cur_end = reinterpret_cast<uintptr_t>(dlinfo->dli_fbase) + cur_phdr->p_vaddr +
                            cur_phdr->p_memsz;
        uintptr_t cur_page_end = SH_UTIL_PAGE_END(cur_end);
        uintptr_t cur_file_end =
                reinterpret_cast<uintptr_t>(dlinfo->dli_fbase) + cur_phdr->p_vaddr +
                cur_phdr->p_filesz;
        uintptr_t cur_file_page_end = SH_UTIL_PAGE_END(cur_file_end);
        uintptr_t next_page_start = (NULL == next_phdr ? cur_page_end : SH_UTIL_PAGE_START(
                (uintptr_t) dlinfo->dli_fbase + next_phdr->p_vaddr));
        ...
        if ((gap.need_fill_zero && gap.end > gap.start + 0x10) ||
            (!gap.need_fill_zero && gap.end > gap.start)) {
            SH_LOG_INFO(
                    "exit: gap, %" PRIxPTR " - %" PRIxPTR " (load_bias %" PRIxPTR ", %" PRIxPTR " - %" PRIxPTR
                    "), NFZ %d, READABLE %d",
                    gap.start, gap.end, (uintptr_t) dlinfo->dli_fbase,
                    gap.start - (uintptr_t) dlinfo->dli_fbase,
                    gap.end - (uintptr_t) dlinfo->dli_fbase, gap.need_fill_zero ? 1 : 0,
                    gap.readable ? 1 : 0);
...
static bool sh_exit_is_elf_loaded_by_kernel(uintptr_t pc) {
    if (NULL == sh_exit_app_process_info.dlpi_phdr) {
        return true;
    }

    if (sh_exit_is_in_elf_range(pc, &sh_exit_app_process_info)) {
        return true;
    }

    if (NULL == sh_exit_linker_info.dlpi_phdr) {
        return true;
    }

    if (sh_exit_is_in_elf_range(pc, &sh_exit_linker_info)) {
        return true;
    }

    if (NULL != sh_exit_vdso_info.dlpi_phdr) {
        if (sh_exit_is_in_elf_range(pc, &sh_exit_vdso_info)) {
            return true;
        }
    }
...
static int sh_exit_fill_zero(uintptr_t start, uintptr_t end, bool readable, xdl_info_t *dlinfo) {
    ...
    if (!readable) {
        if (0 != sh_util_mprotect(start, size, PROT_READ | PROT_WRITE | PROT_EXEC)) {
            return -1;
        }
        set_prot_rwx = true;
    }
    ...
    if (*((uint64_t *) start) != SH_EXIT_CLEAR_FLAG) {
        SH_LOG_INFO(
                "exit: gap fill zero, %" PRIxPTR " - %" PRIxPTR " (load_bias %" PRIxPTR ", %" PRIxPTR
                " - %" PRIxPTR "), READABLE %d",
...
static int sh_exit_try_alloc_in_library(uintptr_t *exit_address, uintptr_t pc, xdl_info_t *dlinfo,
                                        uint8_t *exit,
                                        size_t exit_len, size_t range_low, size_t range_high,
                                        uintptr_t start,
                                        uintptr_t end) {
    ...
    for (uintptr_t cur = start; cur <= end; cur += exit_len) {
        if (!sh_exit_is_zero(cur, exit_len)) {
            continue;
        }

        if (0 != sh_util_mprotect(cur, exit_len, PROT_READ | PROT_WRITE | PROT_EXEC)) {
            return -1;
        }
        ...
        *exit_address = cur;
        SH_LOG_INFO(
                "exit: in-library alloc, at %" PRIxPTR " (load_bias %" PRIxPTR ", %" PRIxPTR "), len %zu",
...
static int sh_exit_alloc_in_library(uintptr_t *exit_address, uintptr_t pc,
                                    xdl_info_t *dlinfo, uint8_t *exit, size_t exit_len,
    ...
    SH_SIG_TRY(SIGSEGV, SIGBUS)
            {
                ...
                size_t gaps_used = sh_exit_get_gaps(dlinfo, gaps, SH_EXIT_GAPS_CAP,
                                                    elf_loaded_by_kernel);
                for (size_t i = 0; i < gaps_used; ++i) {
                    if (gaps[i].need_fill_zero) {
                        if (0 != sh_exit_fill_zero(gaps[i].start, gaps[i].end, gaps[i].readable,
                                                   dlinfo)) {
                            return -1;
                        }
                    }
                    ...
                    if (0 ==
                        (r = sh_exit_try_alloc_in_library(exit_address, pc, dlinfo, exit, exit_len,
                                                          range_low, range_high, gaps[i].start,
                                                          gaps[i].end))) {
                        break;
                    }
                }
...
static int sh_exit_free_in_library(uintptr_t exit_address, uint8_t *exit, size_t exit_len) {
    int r;
    ...
    SH_SIG_TRY(SIGSEGV, SIGBUS)
            {
                if (0 != memcmp((void *) exit_address, exit, exit_len)) {
                    r = SHADOWHOOK_ERRNO_UNHOOK_EXIT_MISMATCH;
                    goto err;
                }
                if (0 != sh_util_mprotect((uintptr_t) exit_address, exit_len,
                                          PROT_READ | PROT_WRITE | PROT_EXEC)) {
                    r = SHADOWHOOK_ERRNO_MPROT;
                    goto err;
                }
                memset((void *) exit_address, 0, exit_len);
                sh_util_clear_cache((uintptr_t) exit_address, exit_len);
                r = 0;
                err:;
            }
        SH_SIG_CATCH()
...
int sh_exit_alloc(uintptr_t *exit_address, uint16_t *exit_type, uintptr_t pc, xdl_info_t *dlinfo,
                  uint8_t *exit, size_t exit_len, size_t range_low, size_t range_high) {
    int r;

    *exit_type = SH_EXIT_TYPE_OUT_LIBRARY;
    r = sh_exit_alloc_out_library(exit_address, pc, dlinfo, exit, exit_len, range_low, range_high);
    if (0 == r) {
        goto ok;
    }

    *exit_type = SH_EXIT_TYPE_IN_LIBRARY;
    r = sh_exit_alloc_in_library(exit_address, pc, dlinfo, exit, exit_len, range_low, range_high);
    if (0 == r) {
        goto ok;
    }
    ...
    SH_LOG_INFO("exit: alloc %s library, exit %" PRIxPTR ", pc %" PRIxPTR ", distance %" PRIxPTR
                        ", range [-%zx, %zx]",
                (*exit_type == SH_EXIT_TYPE_OUT_LIBRARY ? "out" : "in"), *exit_address, pc,
                (pc > *exit_address ? pc - *exit_address : *exit_address - pc), range_low,
                range_high);
    return 0;
}

int sh_exit_free(uintptr_t exit_address, uint16_t exit_type, uint8_t *exit, size_t exit_len) {
    if (SH_EXIT_TYPE_OUT_LIBRARY == exit_type) {
        sh_exit_free_out_library(exit_address);
        return 0;
    } else {
        return sh_exit_free_in_library(exit_address, exit, exit_len);
    }
}
